...
def save_dict_logs(data_dict_logs, folder):
    err_train_main_effect_training = np.array(data_dict_logs["err_train_main_effect_training"])
    err_train_main_effect_tuning = np.array(data_dict_logs["err_train_main_effect_tuning"])
    err_train_interaction_training = np.array(data_dict_logs["err_train_interaction_training"])
    err_train_interaction_tuning = np.array(data_dict_logs["err_train_interaction_tuning"])
    err_val_main_effect_training = np.array(data_dict_logs["err_val_main_effect_training"])
    err_val_main_effect_tuning = np.array(data_dict_logs["err_val_main_effect_tuning"])
    err_val_interaction_training = np.array(data_dict_logs["err_val_interaction_training"])
    err_val_interaction_tuning = np.array(data_dict_logs["err_val_interaction_tuning"])
    main_effect_val_loss = np.array(data_dict_logs["main_effect_val_loss"])
    interaction_val_loss = np.array(data_dict_logs["interaction_val_loss"])
    active_main_effect_index = np.array(data_dict_logs["active_main_effect_index"])
    active_interaction_index = np.array(data_dict_logs["active_interaction_index"])
    save_path = folder + "data_dict_logs.mat"
    sio.savemat(save_path, {'err_train_main_effect_training': err_train_main_effect_training, 'err_train_main_effect_tuning': err_train_main_effect_tuning, 'err_train_interaction_training': err_train_interaction_training,
                            'err_train_interaction_tuning': err_train_interaction_tuning, 'err_val_main_effect_training': err_val_main_effect_training, 'err_val_main_effect_tuning': err_val_main_effect_tuning,
                            'err_val_interaction_training': err_val_interaction_training, 'err_val_interaction_tuning': err_val_interaction_tuning, 'main_effect_val_loss': main_effect_val_loss,
                            'interaction_val_loss': interaction_val_loss, 'active_main_effect_index': active_main_effect_index, 'active_interaction_index': active_interaction_index})  # 保存需要的东西

def save_dict_global_importance(data_dict_global, folder):
    all_ir = []
    for key, item in data_dict_global.items():
            all_ir.append(item["importance"])
    importance = np.array(all_ir)
    save_path = folder + "global_importance.mat"
    sio.savemat(save_path, {'global_importance': importance})

def save_dict_global_density(data_dict_global, folder):
    save_path = folder + "global_density"
    a = 0
    for key, item in data_dict_global.items():
        a = a + 1
        global_density = item["outputs"]
        sio.savemat(save_path + str(a) + '.mat', {'global_density' + str(a): global_density})